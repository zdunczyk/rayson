...
(function(bundle) {
    ...
    function newNode(type, length, value, lvl, pos, key) {
        return rayson.util.setNodeFlag(new Node(type, length, value, lvl, pos, key));
    }

    function Node(type, length, value, lvl, pos, key) {
        this.type = type;
        this.length = length;
        this.value = value;
        this.level = lvl;
    ...
    function iterate(tree_data) {
        var stack = [ { level: 0, position: 0, node: tree_data } ],
            current,
            result = [];
        ...
        while(stack.length !== 0) {
            ...
            if(rayson.util.isNode(current.node)) {
                ...
                stack.push({ 
                    node: newNode(
                            NODE_TYPE_ARRAY, 
                            current.node.length, '', current.level, current.position, current.key
                    ) 
                });
                ...
                stack.push({
                    node: newNode(
                            NODE_TYPE_INTERNAL, 
                            0, current.node, current.level, current.position, current.key
                    )
                });  
                ...
                var current_instance = newNode(
                        NODE_TYPE_OBJ, 
                        0, '', current.level, current.position, current.key
                );
                ...
                for(var child in current.node) {
                    if(current.node.hasOwnProperty(child)) {
                        stack.push({
                            ...
                            level: current.level + 1,
                            position: current_instance.length
                        });    
